...
export default async function UserDetailPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return (
    <>
      ...
      <section className="card detail-card audit-card"><h2>{zh ? "管理员操作记录" : "Admin activity"}</h2>{audit.length ? <div className="table-scroll"><table className="admin-table"><thead><tr><th>{zh ? "时间" : "Time"}</th><th>{zh ? "操作者" : "Actor"}</th><th>{zh ? "操作" : "Action"}</th><th>{zh ? "结果" : "Result"}</th><th>IP</th></tr></thead><tbody>{audit.map((entry) => <tr key={entry.id}><td>{date.format(new Date(entry.createdAt))}</td><td>{entry.actorName}<small className="table-subtext">{entry.actorIdentifier}</small></td><td><code>{entry.action}</code></td><td><span className={`badge ${entry.result === "success" ? "badge-success" : "badge-danger"}`}>{entry.result}</span></td><td>{entry.ipAddress}</td></tr>)}</tbody></table></div> : <div className="admin-empty compact">{zh ? "暂无操作记录" : "No admin activity"}</div>}</section>
    </>
  );
}
