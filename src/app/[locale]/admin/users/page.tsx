...
export default async function UsersPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<SearchValues> }) {
  ...
  return (
    <>
      <header className="admin-page-heading"><div><span className="eyebrow">{zh ? "账号与权限" : "Accounts & access"}</span><h1>{zh ? "用户管理" : "User management"}</h1><p>{zh ? `共 ${result.total} 名用户，可搜索、查看并安全管理其状态与角色。` : `${result.total} users. Search, inspect, and safely manage status and roles.`}</p></div></header>
      <form className="user-filters card" method="get">
        <label className="user-search"><Search size={17} /><span className="sr-only">{zh ? "搜索用户" : "Search users"}</span><input name="q" defaultValue={result.filters.query} placeholder={zh ? "按姓名或邮箱搜索…" : "Search by name or email…"} maxLength={100} /></label>
