...
test("工具管理覆盖新增、执行、访问策略、禁用和删除", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("英文 Description").fill("Format shared team JSON with the production formatter engine.");
  await page.getByLabel("中文 Description").fill("使用生产格式化引擎处理团队共享的 JSON 数据。");
  ...
  await page.getByLabel("英文 SEO 描述").fill("Format shared JSON locally with the team formatter.");
  await page.getByLabel("中文 SEO 描述").fill("在浏览器本地格式化团队共享的 JSON 数据。");
  ...
  await expect(page.getByRole("heading", { name: "Sprint 7 Team JSON" })).toBeVisible();
  await page.goto(`/zh/tools/${slug}`);
  await hydrated(page);
  await expect(page.getByRole("heading", { name: "Sprint 7 团队 JSON", level: 1 })).toBeVisible();
  ...
  await expect(page.getByText("工具配置已保存。")).toBeVisible();
  await page.goto(`/zh/tools/${slug}`);
  await expect(page.getByRole("heading", { name: "此工具需要付费权限" })).toBeVisible();
  ...
  await expect(page.getByText("未找到您访问的工具或页面。", { exact: false })).toBeVisible();
...
test("系统设置实时控制公开站点并可安全恢复", async ({ page }, testInfo) => {
  ...
  await expect(page.locator(".footer-bottom")).toContainText("Sprint 7 动态页脚");
  ...
  await page.getByLabel("中文 Description").fill("打开即用、快速且隐私优先的在线开发工具箱。");
  await page.getByLabel("English Description").fill("Fast, private developer utilities for formatting, encoding, testing, and everyday engineering work.");
  await page.getByLabel("中文 Footer").fill("快速、隐私优先的日常开发工具。");
  await page.getByLabel("英文 Footer").fill("Fast, private utilities for everyday development work.");
  ...
  await expect(page.getByText("系统设置已保存并生效。")).toBeVisible();
  await page.goto("/zh");
  await expect(page.getByRole("heading", { name: "常用开发工具，打开即用。" })).toBeVisible();
});
