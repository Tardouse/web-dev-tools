...
test("数据大小工具区分 bit、Byte、SI 与 IEC 单位", async ({ page }) => {
  ...
  const sourceUnit = page.getByLabel("数据大小原单位");
  await sourceUnit.selectOption("bit");
  await expect(sourceUnit).toHaveValue("bit");
...
test("文本统计与大小写转换覆盖英文字符和首字母大写", async ({ page }) => {
  ...
  const textCounterInput = page.getByLabel("输入或粘贴文本进行分析…");
  await textCounterInput.fill("ABC 世界 123");
  await expect(textCounterInput).toHaveValue("ABC 世界 123");
  ...
  const caseFormat = page.getByLabel("转换格式");
  await caseFormat.selectOption("capitalize");
  await expect(caseFormat).toHaveValue("capitalize");
...
test("行工具支持清理、自然排序、反转及行号往返", async ({ page }) => {
  ...
  const cleanLinesInput = page.getByLabel("待清理文本");
  await cleanLinesInput.fill("alpha\n\nalpha\nbeta\n  ");
  await expect(cleanLinesInput).toHaveValue("alpha\n\nalpha\nbeta\n  ");
  ...
  const sortableLines = page.getByLabel("待处理行");
  await sortableLines.fill("item10\nitem2\nitem1");
  await expect(sortableLines).toHaveValue("item10\nitem2\nitem1");
  ...
  await expect(page.getByLabel("排序方式")).toHaveValue("reverse");
  await sortableLines.fill("one\ntwo\nthree");
  await expect(sortableLines).toHaveValue("one\ntwo\nthree");
  await page.getByRole("button", { name: "处理行" }).click();
  await expect(page.locator(".editor-output")).toHaveText("three\ntwo\none");

  await openTool(page, "line-numberer");
  const lineNumberInput = page.getByLabel("起始行号");
  await lineNumberInput.fill("8");
  await expect(lineNumberInput).toHaveValue("8");
  await page.getByRole("checkbox", { name: "补零对齐" }).check();
  const numberedLinesInput = page.getByLabel("文本行");
  await numberedLinesInput.fill("alpha\nbeta\ngamma");
  await expect(numberedLinesInput).toHaveValue("alpha\nbeta\ngamma");
  ...
  await numberedLinesInput.fill("1. alpha\n2: beta\n3\tgamma");
  await expect(numberedLinesInput).toHaveValue("1. alpha\n2: beta\n3\tgamma");
...
test("文本去重、合并与拆分支持主要模式", async ({ page }) => {
  ...
  const splitMode = page.getByLabel("拆分分隔方式");
  await splitMode.selectOption("custom");
  await expect(splitMode).toHaveValue("custom");
