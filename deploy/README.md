...
```bash
NATIVE_PORT=9000 DATA_DIR=/srv/devtoolbox/data ./scripts/install-no-docker.sh
ENV_FILE=/etc/devtoolbox.env NATIVE_DIR=/opt/devtoolbox-runtime ./scripts/install-no-docker.sh
```
...
5. Set `NEXT_PUBLIC_SITE_URL=https://tools.example.com` in `.env.native` and rerun `./scripts/install-no-docker.sh`, or set it in `.env` and rerun `./scripts/deploy.sh` for Docker. This ensures canonical metadata is built correctly.
