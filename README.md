...
This installer runs `npm ci` and `next build` directly on the host, then starts
the standalone Node.js server. It never invokes Docker or Docker Compose.
`scripts/install-native.sh` and `scripts/install.sh` remain compatible aliases
for this native deployment path. Docker deployment remains available separately
